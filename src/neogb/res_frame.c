...
/* Ascending ring order; the hash index breaks ties only for equal
 * monomials, which cannot both survive minimalization, so it is there
 * purely to make the sort deterministic.
 *
 * The direction is the one free choice in the whole construction, and it
 * is worth a word.  Schreyer's theorem attaches the lead term of the
 * S-pair of l and k to whichever index is larger, so the elements early
 * in a block are the ones being divided *into*: putting the small
 * monomials first keeps every colon quotient m_l/gcd(m_l,m_k) small and
 * so keeps the number of minimal generators down.  Reversing this is
 * still a correct frame, only a bigger one -- for (x^2, xy, y^3) it grows
 * the frame from 1,3,2 to 1,3,3,1, resolving a projective dimension 2
 * module in three steps.  Ascending is also what Macaulay2's
 * res(..., Strategy => Nonminimal) does, which is what the corpus in
 * test/neogb/res compares against. */
static int res_frame_cmp_mono_asc(
        const void *a,
        const void *b,
        void *htp
        )
{
    ...
    const int c = dispatch_monomial_cmp(x, y, ht);
    if (c != 0) {
        return c > 0 ? 1 : -1;
    }
    return x < y ? -1 : (x > y ? 1 : 0);
        ...
        )
{
    ...
    return res_frame_cmp_mono_asc(&x->mono, &y->mono, htp);
}
        ...
        )
{
    ...
    f->nlv   = 0;
    f->bad   = 0;
    /* over a polynomial ring in n variables every frame stops at level n,
     * by Hilbert's syzygy theorem applied to the module of lead terms.
     * Level 1 is the Gröbner basis and is always built, so one is the
     * floor. */
    f->maxlv = maxlevel > 0 ? (len_t)maxlevel : (len_t)md->nvars;
    if (f->maxlv < 1) {
        f->maxlv = 1;
    }
    ...
    f->lv = (res_level_t *)calloc(
            (unsigned long)f->maxlv + 1, sizeof(res_level_t));
    if (f->lv == NULL) {
        ...
        )
{
    ...
    if (f == NULL || f->nlv < 2 || f->nlv > f->maxlv) {
        return 0;
    }
        ...
        )
{
    ...
    while (f->nlv >= 2 && f->nlv <= f->maxlv && f->bad == 0) {
